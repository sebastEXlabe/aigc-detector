...
#### 深流增强版重训2（2026-09-02，提升C-ReD多样/未知LLM泛化）
- `finetune_roberta_wsl_aug.py`（增强版采样再升级）：
  - **AI 正样本 16000 句**：guard 后 train_unified AI(3390) + **C-ReD 全部8个中文学术生成器 10000 句** + HC3中文 4500 + M4-zh-qa 1500
  - **human 负样本 36477 句**：cnki文献库 + self_train高置信真人句 + thesis + train_unified human（池=36477），**human:AI ≈ 2.28:1**
- 微调结果（WSL GPU 4060Ti, 3 epoch, 总52477句）：
  | epoch | loss | acc | f1 | AUC |
  |---|---|---|---|---|
  | 1 | 0.1691 | 0.9460 | 0.9127 | 0.9897 |
  | 2 | 0.0877 | 0.9517 | 0.9226 | 0.9919 |
  | 3 | 0.0470 | 0.9513 | 0.9230 | 0.9921 |
- 模型已更新至 `models/roberta_ft/`（`model.safetensors` 2026-09-02 23:39 重写，409MB）。
